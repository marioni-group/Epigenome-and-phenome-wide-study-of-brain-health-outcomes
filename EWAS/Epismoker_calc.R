...
### DOING THIS THE MANUAL WAY ALSO GIVES SAME RESULTS IN LBC - as sanity check!

####################################################################################

# Ill try with the LBC where we know what the outcome should be, so we know were doing it right for STRADL
# Honestly fuck this! 

# Download Illig data from here:
# Download this and run source("SSc.R")
# Download this and run source("epismoker.R")

# Illig data is used as the Elliott ref
# Which is pulled in with "SSc"

# Then try epismoker(data, method="SSc")

# Set working directory to where files are for epismoker 
setwd("/Cluster_Filespace/Marioni_Group/Danni/Stradl_markers/Epismoker_stradl/")
...
# Try to calculate 
load("/Cluster_Filespace/Marioni_Group/Danni/Stradl_markers/Epismoker_stradl/Illig_data.RData")
load("/Cluster_Filespace/Marioni_Group/LBC/LBC_methylation/Beta_3525_norm_bgcorrect_0.001BetaThreshold_probefilter.RObject")
smok <- epismoker(dat, method="SSc")


# Read in the old version
smok_old <- readRDS("/Cluster_Filespace/Marioni_Group/Danni/lbc_epismoker.rds")
